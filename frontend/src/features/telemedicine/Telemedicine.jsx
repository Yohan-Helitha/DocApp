...
export default function Telemedicine({ navigate }) {
  ...
  const loadSessions = async (tab) => {
    ...
    try {
      const res = await authedFetch(
        `/api/v1/telemedicine/sessions?status=${encodeURIComponent(tab)}`,
        "GET",
      );
      if (
        res.status >= 200 &&
    ...
    const run = async () => {
      ...
      try {
        ...
        const joinRes = await authedFetch(
          `/api/v1/telemedicine/sessions/${sessionId}/join-token`,
          "POST",
          {
            role,
          },
  ...
  const getLink = async (session) => {
    ...
    try {
      const role = getRoleFromToken();
      const res = await authedFetch(
        `/api/v1/telemedicine/sessions/${session.session_id}/join-token`,
        "POST",
        {
          role,
        },
  ...
  const start = async (session) => {
    ...
    try {
      const startRes = await authedFetch(
        `/api/v1/telemedicine/sessions/${session.session_id}/start`,
        "PUT",
      );
  ...
  const end = async (session) => {
    ...
    try {
      const res = await authedFetch(
        `/api/v1/telemedicine/sessions/${session.session_id}/end`,
        "PUT",
      );
  ...
  const remove = async (session) => {
    ...
    try {
      const res = await authedFetch(
        `/api/v1/telemedicine/sessions/${session.session_id}`,
        "DELETE",
      );
      if (res.status < 200 || res.status >= 300) {
        ...
        <main className="p-8 min-h-screen">
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Appointment:{" "}
              <span className="font-mono">{fmtShort(appointmentId)}</span>
            </p>
          ...
          <main className="md:ml-64 p-8 min-h-screen">
            <div className="max-w-5xl mx-auto">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#0b9385]">
                    Doctor Workspace
                  </p>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Create Jitsi sessions, generate meeting links, and control
                    consultation status.
                  </p>
                </div>
                <button
                  ...
                  onClick={() =>
                    navigate
                      ? navigate("/success/doctor")
                      : (window.location.hash = "/success/doctor")
                  }
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      ...
                      {sessions.map((s) => {
                        ...
                        const isEnded =
                          String(s.session_status || "").toLowerCase() ===
                          "ended";

                        const canJoinNow = appt
                          ? isWithinSlotWindow(appt)
                          : false;
                        const timeLocked = !canJoinNow;
                        const timeLockedTitle = timeLocked
                          ? "Locked until the appointment time window."
                          : undefined;
                          ...
                          >
                            <td className="py-4 pr-4">
                              ...
                              <div className="text-xs text-slate-500 font-mono">
                                Appt: {fmtShort(s.appointment_id)}
                              </div>
                            </td>
