          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sticky top-8">
              <div className="flex flex-col items-center mb-6">
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          const reader = new FileReader();
                          reader.onloadend = () => {
                            setPatientData(prev => ({ ...prev, profileImage: reader.result }));
                          };
                          reader.readAsDataURL(file);
                        }
                      }}
                ...
                <p className="text-sm text-slate-500">Patient ID: #PT12345</p>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-slate-900 font-medium">{patientData.emergencyContactName}</p>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Phone</label>
                  <p className="text-slate-900 font-medium">{patientData.emergencyContactPhone}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-8 py-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Update Profile Info</h3>
                <p className="text-slate-600 text-sm mt-1">Adjust your personal details and emergency contact information.</p>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-8">
              ...
              <div className="mt-10 flex gap-4 border-t border-slate-200 pt-8">
                <button 
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-8 py-3.5 border border-slate-300 text-slate-700 font-bold rounded-xl hover:bg-slate-100 transition-all active:scale-[0.98]"
                >
                  Discard
                </button>
                <button 
                  type="submit"
                  className="flex-1 py-3.5 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 shadow-lg shadow-primary/25 transition-all active:scale-[0.98] uppercase tracking-wider text-sm"
                >
